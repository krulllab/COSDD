...
from .probability_density_functions import log_normal_pdf, DiscreteLogistic
...
class PixelCNN(nn.Module):
    ...
    def extract_params(self, params):
        logits = params[:, 0::3].unfold(1, self.n_components, self.n_components)
    ...
    def loglikelihood(self, x, params):
        logits, loc, log_scale = self.extract_params(params)
        if self.discretised:
            log_p = self.discrete_logistic_pmf(x, loc, log_scale, logits)
        else:
            ...
            log_weighted_p_per_component = logits + log_p_per_component
